#let introduction_content = [
  ...
  - Copy-and-patch @jit compilation, as proposed in @pep:short 744 @pythonjit, and with the latest stable implementation as of writing in Python 3.14 @python314.
    ...
    This is far from an exhaustive list. Many other approaches exist, such as the LLVM-based @jit compiler Numba @numba, the .NET-based IronPython @ironpython, the Python superset language Mojo @mojo, etc., but due to the limited scope of this paper and due to the fact that these are either too specialized, not as frequently maintained, or not as mature as the ones mentioned above, they will not be included in this comparison.
  ...
  #quote(
    [In a nutshell, @copy_and_patch allows a high-quality template JIT compiler to be generated from the same @dsl:short used to generate the rest of the interpreter.],
    attribution: <pythonjit>,
    block: true,
  )
